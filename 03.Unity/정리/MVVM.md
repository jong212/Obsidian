...
>[!NOTE] TopLeftUIView.cs
## View 동작 순서
1. 뷰는 뷰모델을 들고있어야 하고
2. 초기화 단계에서 뷰모델의 PropertyChanged 이벤트에 OnPropertyChangeds 메서드를 등록해 두어야 한다.
   ![[Pasted image 20240816013553.png]]
   미리 등록하는 이유는 뷰에있는 OnPropertyChangeds 메서드가 호출 되기 위함인데 예를들어 레벨업을 했다고 가정하고 그 레벨을 표시하는 View가 여러곳이라 하면 그 View를 표시하는 모든 곳에 한 번에 전파가 되어야 하겠지?
   ![[Pasted image 20240816013413.png]]
   그래서 아래의 뷰모델(원랜 뷰모델에 아래 함수가 있어야 하는데 여러 뷰에서 아래 뷰모델을 상속 받게 하기 위해 뷰모델 베이스를 따로 만들고 정의 했음)에 있는 OnPropertyChanged 이벤트에 등록을 해놓는 것이다 한 마디로 뷰는 이벤트를 등록시켜놓고 전달만 받기를 기다리는 입장이 되는 것이다 
   ![[Pasted image 20240816015853.png]]
   그럼 내가 구독해놓은 이벤트가 언제 실행되는지가 궁금해질텐데 이건 뷰모델 동작 순서 챕터에서 알아보자
   
**위 내용을 요약해 보자면 뷰에서는 아래와 같이
1. 뷰에 대한 요소만 들고 있어야 하고
2. 뷰모델을 들고 있어야 하고
3. 뷰모델이 있는 이벤트에 뷰에 있는 OnPropertyChanged 메서드를 등록 시켜야 한다
4. 특징으로는 모노를 상속받고 OnDisabled 일 땐 구독을 해제한다
